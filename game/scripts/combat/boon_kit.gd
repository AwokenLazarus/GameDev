...
## Deep-pact passives and kit transforms live in MWPactKit (MW-005); this kit asks it
## for pact crits, the execute bonus, the Debt cap and the execute rally bell.
...
func _tick_dash(delta: float) -> void:
	...
	if has("dust_dash"):
		...
		if _trail_t <= 0.0:
			_trail_t = 0.05
			smoke(p.global_position, 26.0, 2.0, 1.5)
		...
		"attack":
			...
			if has("church_smite"):
				...
				if _attack_count % 3 == 0:
					var t := foe_ahead(220.0, 0.9)
					smite(t.global_position if t else p.global_position + p.facing * 110.0)
		"special":
			if has("veyra_pointe"):
				var t := foe_ahead(140.0, 1.05)
				if t:
					afterimage(p.global_position, C_VEYRA)
...
func _on_dash_started(_dir: Vector2) -> void:
	...
	if has("petition_caltrops"):
		var f := field(_dash_from, 46.0, 4.0, C_PETITION)
		f.interval = 0.5
...
func before_hit(foe: Node, dmg: float, slot: String) -> Dictionary:
	...
	if slot in COMBAT_SLOTS:
		...
		if has("petition_manifest") and _is_winding(foe):
			dmg += p.base_hit(3.0)
		if p.pacts.wants_crit(foe, slot):
			crit = true
...
func after_hit(foe: Node, dmg: float, slot: String, heavy: bool, crit: bool) -> void:
	...
	match slot:
		...
		"smite":
			if st.condemned():
				var next := nearest_foe((foe as Node2D).global_position, 200.0, foe)
				if next:
	...
	if has("church_fatherslight") and MWBoonStatus.is_general(foe) and slot in COMBAT_SLOTS:
		st.condemn(5.0)
	if slot in ["attack", "echo"] and (has("petition_execute") or p.pact() == "red_petition"):
		try_execute(foe)
...
## `mult` scales it (the Guillotine Drop executes at double threshold).
func execute_threshold(foe: Node, mult: float = 1.0) -> float:
...
func try_execute(foe: Node, mult: float = 1.0) -> bool:
	var h: Health = foe.get_node_or_null("Health")
	if h == null or not h.is_alive():
		return false
	if h.hp / maxf(h.max_hp, 1.0) >= execute_threshold(foe, mult):
		return false
	execute(foe)
	return true
...
func on_kill(foe: Node, slot: String, crit: bool) -> void:
	...
	var st := MWBoonStatus.peek(foe)
	if p.debt > 0.0:
		## The Blood Ledger: kills in Debt pay back 5 more.
...
func before_hurt(amount: float, from: Vector2) -> float:
	...
	if p.wards > 0:
		p.wards -= 1
		ring(p.global_position, 34.0, C_CHURCH)
...
func after_hurt(_amount: float) -> void:
	if has("dust_sidestep") and _cinder_cd <= 0.0:
		_cinder_cd = 8.0
		smoke(p.global_position, 50.0, 1.5, 1.5)
		p.dodge_cd = 0.0
...
func try_lethal(amount: float) -> bool:
	...
	p.health.hp = 1.0
	add_debt(owed)
	ring(p.global_position, 40.0, C_VEYRA)
	return true
...
func debt_cap() -> float:
	return DEBT_CAP_PACT if p.pact() == "house_veyra" else DEBT_CAP
...
func _cast_arrive(pos: Vector2) -> void:
	...
	if has("petition_trap"):
		...
		var f := field(pos, 16.0, 20.0, Color(0.6, 0.6, 0.66))
		f.marker = true
		f.enemy_once = true
		f.interval = 0.05
		f.on_enemy = func(e: Node) -> void:
...
func _dust_devil(pos: Vector2) -> void:
	var f := field(pos, 90.0, 2.0, C_DUST)
	f.interval = 0.5
	f.on_enemy = func(e: Node) -> void:
		var n := e as Node2D
...
func _scrap_cache(pos: Vector2) -> void:
	var f := field(pos, 14.0, 12.0, C_DUST)
	f.marker = true
	f.pickup = true
	f.on_player = func(pl: Node) -> void:
		pl.refund_cast(1)
...
func _couture_image(pos: Vector2) -> void:
	afterimage(pos, C_VEYRA_GILT, 0.3)
...
func sabotage(foe: Node, seconds: float) -> void:
...
func ring(pos: Vector2, r: float, tint: Color) -> void:
	_VFX.ring(p.get_parent(), pos, r, tint)
...
func _crit_fx(foe: Node) -> void:
	...
	if av and av.has_method("flash"):
		## The Blood Ledger crits flash oxblood instead of gold-red.
		av.flash(Color(1.6, 0.2, 0.3) if p.pact() == "house_veyra" else Color(1.9, 0.75, 0.3), 0.14)
